...
export function ForecastingPanel({
  ...
  const generateChartData = () => {
    ...
    for (let year = startYear; year <= endYear; year++) {
      const yearQuarters = timeSeries.timestamps.filter((t) =>
        t.startsWith(`${year}-`),
      );
      ...
      <div className="mb-4 md:mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
          <div>
            <h2 className="text-lg md:text-xl font-bold text-foreground">
              DeepAR Time Series Forecast
            </h2>
            <p className="text-xs md:text-sm text-muted-foreground">
              Probabilistic forecasting using Amazon DeepAR for land cover
              change prediction
            </p>
          </div>
          {selectedBarangay && (
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-lg px-3 md:px-4 py-2 md:py-3 mt-3 md:mt-4">
          <p className="text-xs md:text-sm text-foreground">
            <span className="font-semibold text-primary">2026 Forecast:</span>{" "}
            Predictions generated from historical data (2016-2025) using DeepAR
            probabilistic time series model trained on Dynamic World V1 land
            cover observations.
          </p>
        </div>
      </div>
                ...
                {tableData.map((row) => (
                  <tr key={row.id} className="border-b border-border/50">
                    ...
                    <td className="py-3 px-4 text-center">
                      <span
                        className={`flex items-center justify-center gap-1 ${row.change >= 0 ? "text-green-500" : "text-red-500"}`}
                      >
                        {row.change >= 0 ? (
