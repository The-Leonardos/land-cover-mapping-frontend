...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setSelectedYear((prev) => (prev >= 2026 ? 2016 : prev + 1))
        }, 800)
    ...
    return (
        <div className="flex h-screen w-full flex-col bg-background text-foreground overflow-hidden">
                ...
                <div className="flex items-center justify-between relative">
                    ...
                    <div className="flex items-center gap-2 md:gap-4">
                        ...
                        <div className="hidden sm:block">
                            <h1 className="text-sm md:text-base font-bold text-foreground tracking-tight">Baguio City Land Cover</h1>
                            <p className="text-xs text-muted-foreground font-medium">Monitoring Agent</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button className="hidden sm:block p-2 md:p-2.5 hover:bg-muted/80 rounded-lg transition-colors" title="User Profile">
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <div className="flex-1 relative">
                    {activeTab === "map" ? (
                        <InteractiveMapGrid
                            ...
                            year={selectedYear}
                            shouldRenderSegmentation={selectedYear === 2026 && activeLayers.has("segmentation")}
                        />
                    ) : (
                        <ForecastingPanel
                            selectedBarangay={selectedBarangay}
                ...
                {selectedBarangay && activeTab === "map" && (
                    <div className={`lg:hidden fixed inset-x-0 bottom-0 z-30 transition-transform duration-300 ease-out ${showMobilePanel ? "translate-y-0" : "translate-y-[calc(100%-4rem)]"}`}>
                        {/* Pull Tab */}
                        <button
                        ...
                        >
                            <ChevronUp className={`h-4 w-4 text-muted-foreground transition-transform ${showMobilePanel ? "rotate-180" : ""}`} />
                            <span className="text-sm font-medium text-foreground">{selectedBarangay}</span>
                        </button>
