          ...
          .map((layer) => (
            <div key={layer.id} className="space-y-2">
              <div className="flex items-start justify-between gap-2 md:gap-3">
                ...
                {/* Toggle Switch — only for toggleable layers */}
                {layer.toggleable && (
                  <button
                    onClick={() => onLayerToggle(layer.id)}
                    className={`w-9 h-5 md:w-10 md:h-6 rounded-full p-0.5 transition-colors flex-shrink-0 ${activeLayers.has(layer.id) ? "bg-primary" : "bg-muted"
                      }`}
                  >
                    <div
                      className={`w-4 h-4 md:w-5 md:h-5 rounded-full bg-background shadow transition-transform ${activeLayers.has(layer.id)
                        ? "translate-x-4"
                        : "translate-x-0"
                        }`}
                    />
